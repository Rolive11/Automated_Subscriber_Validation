"""
validate_subscription_isp_mod_4.py
...
1. EXCEL-TO-CSV AUTO-CONVERSION
   - Automatically detects and converts Excel files (.xlsx, .xls, .xlsm, .xlsb) to CSV
   - Users no longer need to manually convert files
   - Original Excel files backed up with '_original' suffix

2. ORIGROWNUM AUTO-REMOVAL
   - Automatically detects and removes "OrigRowNum" column from both CSV and Excel files
   - Prevents validation errors when users forget to delete Column A
   - Simplified detection: exact column name match only (optimized for performance)
   - Original files backed up before modification

3. ENHANCED LOGGING
   - [EXCEL CONVERSION] logs for Excel-to-CSV operations
   - [CSV CLEANING] logs for CSV OrigRowNum removal
   - [CSV CHECK] logs for CSV file verification
   - Detailed column count and row count tracking

4. UPDATED USER INSTRUCTIONS
   - Customer emails now state: "Excel or CSV format - both work!"
   - Simplified workflow for users

5. BCC EMAIL UPDATES
   - Added tbleeker@regulatorysolutions.us to BCC list

DEPENDENCIES:
=============
- pandas (new requirement)
- openpyxl (new requirement for Excel reading)

FUNCTIONS ADDED:
================
- _detect_and_remove_index_column(): Detects and removes OrigRowNum column
- prepare_subscriber_file(): Main file preparation function (replaces convert_excel_to_csv_if_needed)

IMPACT:
=======
Eliminates 50-75% of user errors related to:
- Forgetting to convert Excel to CSV
- Forgetting to remove OrigRowNum column

See COMPREHENSIVE_SOLUTION_SUMMARY.md for detailed documentation.
"""
...
class TeeOutput:
    ...
    Overwrites the log file on each new run.
    """
    def __init__(self, log_file_path):
        self.terminal = sys.stdout
        self.log_file_path = log_file_path
        # Overwrite file on init (fresh log each run)
    ...
    def write(self, message):
        ...
        # Write to file with timestamp (only for non-empty, non-newline messages)
        if message.strip():
...
# Global variable for console logger
...
def sendEmail(customer, name, emessage, attachment_path=None, subject=None):
    ...
    # Build list of all recipients (To + BCC) for actual SMTP transmission
    all_recipients = [customer] + email_config['bcc_list']
    ...
    with open('validate_subs.log', 'a') as f:
        print(f'User email sent successfully to {customer} (BCC: {", ".join(email_config["bcc_list"])})\n', file=f)
...
def sendEmailToAdmin(subject, message, attachment_paths=None,
    ...
    try:
        ...
        # Build list of all recipients (To + BCC) for actual SMTP transmission
        all_recipients = [admin_email] + email_config['bcc_list']
        ...
        with open('validate_subs.log', 'a') as f:
            print(f'Admin email sent successfully to {admin_email} (BCC: {", ".join(email_config["bcc_list"])})\n', file=f)
...
def _detect_and_remove_index_column(df, file_type, org_id):
    ...
    Args:
        ...
        file_type (str): 'EXCEL' or 'CSV' for logging purposes
        org_id (str): Organization ID for logging

    Returns:
        tuple: (modified_df, was_removed, removed_column_name)
    """
    if len(df.columns) == 0:
...
def prepare_subscriber_file(file_path, org_id):
    """
    Prepare subscriber file for validation by:
    1. Converting Excel to CSV if needed
    2. Removing OrigRowNum or other index columns from both Excel and CSV files
    3. Backing up original files when modifications are made

    This function handles the common user error of forgetting to:
    - Convert Excel back to CSV
    - Remove the OrigRowNum column before resubmitting
    ...
    Returns:
        str: Path to cleaned CSV file ready for validation
    """
    ...
    if file_ext in ['.xlsx', '.xls', '.xlsm', '.xlsb']:
        with open('validate_subs.log', 'a') as f:
            print(f'[EXCEL CONVERSION] Org {org_id}: Detected Excel file: {os.path.basename(file_path)}\n', file=f)
            print(f'[EXCEL CONVERSION] Converting to CSV format...\n', file=f)
        ...
        try:
            ...
            df, was_removed, removed_col = _detect_and_remove_index_column(df, 'EXCEL', org_id)

            # Generate CSV path (same location, change extension to .csv)
            ...
            with open('validate_subs.log', 'a') as f:
                print(f'[EXCEL CONVERSION] Successfully converted to: {os.path.basename(csv_path)}\n', file=f)
                print(f'[EXCEL CONVERSION] Original columns: {original_columns}, Final columns: {len(df.columns)}\n', file=f)
                if was_removed:
                    print(f'[EXCEL CONVERSION] Removed column: "{removed_col}"\n', file=f)
                print(f'[EXCEL CONVERSION] Rows: {len(df)}\n', file=f)

            # Backup original Excel file
            backup_path = os.path.splitext(file_path)[0] + '_original' + file_ext
            shutil.move(file_path, backup_path)

            with open('validate_subs.log', 'a') as f:
                print(f'[EXCEL CONVERSION] Original Excel backed up as: {os.path.basename(backup_path)}\n', file=f)

            return csv_path

        except Exception as e:
            with open('validate_subs.log', 'a') as f:
                print(f'[EXCEL CONVERSION ERROR] Failed to convert Excel to CSV: {str(e)}\n', file=f)
            # If conversion fails, return original path and let validation handle the error
            return file_path
    ...
    # Handle CSV files
        ...
        try:
            ...
            # If processing fails, return original path and let validation handle the error
            return file_path

    # Handle other file formats (should not happen, but just in case)
    else:
        with open('validate_subs.log', 'a') as f:
            print(f'[FILE CHECK] Org {org_id}: Unknown file format: {file_ext}\n', file=f)
        return file_path
...
def create_subscription(subfile, filename, isp, periodpath, period, user_email):
    ...
    # Prepare subscriber file (convert Excel to CSV, remove OrigRowNum column)
    subscrfile = prepare_subscriber_file(subscrfile, isp)
...
The Regulatory Solutions Team"""
    ...
    with open(subscrfile) as csv_file:
        ...
        for row in csv_reader:
            ...
            lon = row[2] if row[2] != '' else None
            address = row[3].upper()
            city = row[4].upper()
            state = row[5].upper()
            zip = row[6]
            down = row[7] if row[7] != '' else None
            up = row[8] if row[8] != '' else None
            voip = row[9]
            if voip == '':
                voip = int(0)
                print('voip 0')
            if int(voip) > 0:
                voipneeded = True
            business = row[10] if row[10] != '' else 0
            techname = row[11]
            ...
            # addresses)
            if (lat == '' or lat is None or lon == '' or lon is None) and (
        ...
        if len(addrerr) > 0:
            ...
            print('continuing inserting subs to db')
            if (subsexist == True):
            ...
            print('making subscription_processed.csv')
            ...
            print('opening ' + tmpout) 
            with open(tmpout, 'w') as f_output:
            # Use copy_expert to execute the COPY command and stream to the file
...
def main(ispid,per,user_email):
	# Initialize console logging to file (captures all print statements)
	...
	db_host = os.getenv('DB_HOST', 'broadband.regulatorysolutions.us')
