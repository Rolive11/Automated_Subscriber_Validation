...
from src.utils.file_handling import save_csv, sanitize_for_excel, sanitize_dataframe_for_excel
from src.config.settings import get_validation_threshold, is_address_column, ADDRESS_COLUMNS


def safe_to_excel(df, writer, sheet_name, index=False):
    """
    Write DataFrame to Excel after sanitizing for illegal characters.
    Prevents openpyxl IllegalCharacterError from control characters.
    """
    sanitized_df = sanitize_dataframe_for_excel(df)
    sanitized_df.to_excel(writer, sheet_name=sheet_name, index=index)
...
def assess_file_validation_status(cleaned_df, cell_fills, rows_excluded_from_excel, flagged_cells=None):
    """
    ...
    for (excel_row, excel_col_letter), (priority_level, fill_color) in cell_fills.items():
        ...
        if excel_col_idx <= len(df_columns):
            ...
            if orig_row is not None:
                ...
                for (row_idx, flagged_col_name), cell_data in (flagged_cells or {}).items():
    ...
    try:
        ...
        for k, v in corrected_cells.items():
            ...
            if orig_row is not None and orig_row in orig_row_to_new_idx:
                # Use orig_row directly - it's already the OrigRowNum we need
        ...
        if critical_non_address_errors:
            ...
            file_validation = assess_file_validation_status(
                cleaned_df, temp_cell_fills, all_rows_to_exclude, flagged_cells_converted
            )
        ...
        with pd.ExcelWriter(vr_excel_path, engine="openpyxl") as writer:
            # Summary tab
            safe_to_excel(pd.DataFrame(list(summary.items()), columns=["Metric", "Value"]), writer, "Summary")
            ...
            safe_to_excel(corrections_df, writer, "Corrections")
            ...
            safe_to_excel(smarty_processing_log_df, writer, "Smarty Processing Log")

            # Error Reference tab
            if error_dict is not None:
                safe_to_excel(error_dict, writer, "Error Reference")
                ...
                safe_to_excel(empty_ref_df, writer, "Error Reference")
            ...
            safe_to_excel(filtered_errors_df, writer, "Errors")
            ...
            safe_to_excel(critical_errors_df, writer, "Critical Errors")
            ...
            safe_to_excel(street_ending_df, writer, "Street Ending Errors")
            ...
            safe_to_excel(pobox_df, writer, "PO Box Errors")
            ...
            safe_to_excel(invalid_address_df, writer, "Invalid Address Removals")
            ...
            safe_to_excel(non_unique_df, writer, "Non-Unique Row Removals")
            ...
            safe_to_excel(duplicate_removals_df, writer, "Duplicate Rows Removed")
