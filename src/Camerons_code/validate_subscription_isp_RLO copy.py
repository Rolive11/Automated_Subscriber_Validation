...
def geoCode(address_or_zipcode):
    ...
    lat, lng = None, None
    api_key = 'AIzaSyCx1Ra2b2vobRiTO1Jw-24YhByG8fmx20Q'
...
def sendEmail(customer, name, emessage, attachment_path=None, subject=None):
	...
	message["Bcc"] = 'cscrumconsulting@gmail.com'  # Recommended for mass emails
	...
	with smtplib.SMTP_SSL("smtp.gmail.com", port, context=context) as server:
		server.login("info@regulatorysolutions.us",'janu pvfs tdsq drwv')
		server.sendmail("info@regulatorysolutions.us", customer, text)
...
def sendEmailToAdmin(subject, message, attachment_paths=None, admin_email='rolive@regulatorysolutions.us'):
    ...
    try:
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", port, context=context) as server:
            server.login("info@regulatorysolutions.us", 'janu pvfs tdsq drwv')
            server.sendmail("info@regulatorysolutions.us", admin_email, text)
        ...
        # Don't raise exception - email failure shouldn't break the main process
...
def call_code_a_validation(org_id, period, subscriber_file_path):
    ...
    try:
        ...
        with open('validate_subs.log', 'a') as f:
            print(f'Code A completed with return code: {return_code}\n', file=f)
            if stdout:
...
def create_subscription(subfile,filename,isp,periodpath,period):
	print("subfile ",subfile," filename ",filename," isp ",isp," periodpath ",periodpath)
	...
	if validation_result['status'] == 'invalid':
		# File needs manual review - send corrected Excel to user and stop processing
		print("========================================")
...
The Regulatory Solutions Team"""
		...
		excel_attachment = validation_result['excel_path'] if validation_result['excel_path'] and os.path.exists(validation_result['excel_path']) else None
		custom_subject = f'Subscriber File Validation - Manual Review Required'
		sendEmail(customer, cname, user_message, excel_attachment, custom_subject)
		...
		if excel_attachment:
			...
			with open('validate_subs.log', 'a') as f:
				print(f'Warning: No Excel file available to send to user for org {isp}\n', file=f)
		
		# Update database status
		sql = """Update filer_processing_status set subscription_processed = true, subscription_status = 'validation_failed' where org_id = """ + isp + """ and filing_period = '""" + period + """' """
		cursor.execute(sql)
		conn.commit()
	...
	The Regulatory Solutions Team"""
    	...
    	sendEmail(customer, cname, error_message, None, 'Subscriber File Processing Error')
    
    	# Update database status
    	sql = """Update filer_processing_status set subscription_processed = true, subscription_status = 'validation_error' where org_id = """ + isp + """ and filing_period = '""" + period + """' """
    	cursor.execute(sql)
    	conn.commit()
	...
	if not validation_result['csv_path'] or not os.path.exists(validation_result['csv_path']):
	...
	with open(subscrfile) as csv_file:
		...
		if ncols != 12:
			print("    ERROR: Code A output should have 12 cols but has " + str(ncols));
			# This should not happen if Code A worked correctly
			sql = """Update filer_processing_status set subscription_processed = true, subscription_status = 'format_error' where org_id = """ + isp + """ and filing_period = '""" + period + """' """
			cursor.execute(sql)
			conn.commit()
			return

	# Continue with Code B's database operations (geocoding, tract assignment, etc.)
	now = datetime.now()
	...
	with open(subscrfile) as csv_file:
		...
		if(subsexist == True):
			...
			sql = """Select * into subscribers.subs_""" + str(isp) + """_temp from subscribers.subs_""" + str(isp) + """ where type != 'Active' """;
			cursor.execute(sql)

		sql = """Drop table if exists subscribers.subs_""" + isp
		cursor.execute(sql)
		sql = """CREATE TABLE subscribers.subs_""" + isp + """ (customer text,lat numeric,lon numeric,address text,address2 text,city text,state text,zip text,download numeric,upload numeric,voip_lines_quantity integer,business_customer numeric,technology integer,tech text,tract text, match boolean,bdc_id integer,type text, date timestamp without time zone,notes text)"""
		cursor.execute(sql)
		conn.commit()
		subsarr = []
		state = ''
		...
		for row in csv_reader:
			...
			# GEOCODING: Only geocode if lat/lon are missing (Code A validated addresses)
			if lat == '' or lon == '' and (address != '' and city != '' and state != '' and zip != ''):
				addr = address + ',' + city + ',' + state + ' ' + zip
				(lt,ln) = geoCode(addr)
				if lt is None or ln is None:
					rowerr = line_count + 1
					print("error geocoding row " + str(rowerr))
					errstr = 'error geocoding row ' + str(rowerr) + ' addr: ' + addr
			...
			if lat != '' and lon != '':
				...
				cursor.execute(sql,(customer,lat,lon,address,city,state,zip,down,up,voip,business,tech,techname,tract,'Active',date_time))
				conn.commit();
			line_count += 1
		...
		sql = """create index subs_""" + str(isp) + """_customer_index on subscribers.subs_""" + str(isp) + """ (customer);"""
		cursor.execute(sql)
		conn.commit()
		...
		if len(addrerr) > 0:
			...
			if dirExist == False:
				os.umask(0)
				os.makedirs(periodpath + "/subscription_processed/", mode=0o777)
			isExist = os.path.exists(errf)

			if (isExist):
				os.remove(errf)
			errfil = open(periodpath + "/subscription_processed/processing_errors.txt","a");
			...
			print("sending geocoding error email to  " + customer + " " + cname)
			sendEmail(customer, cname, em_message, None, 'Subscriber File Processing - Geocoding Issues')
			errfil.close()
			
			sql = """Update filer_processing_status set subscription_processed = true, subscription_status = 'geocoding_errors' where org_id = """ + isp + """ and filing_period = '""" + period + """' """
			cursor.execute(sql)
			conn.commit()
			...
			if(subsexist == True):
				sql = """insert into subscribers.subs_""" + isp + """ select * from subscribers.subs_""" + isp + """_temp """
				cursor.execute(sql)
				conn.commit()
			...
			outfil = periodpath + "/subscription_processed/" + isp + "_subscription_processed.csv" 
			dirExist = os.path.exists(periodpath + "/subscription_processed/")
			if dirExist == False:
				os.umask(0)
				os.makedirs(periodpath + "/subscription_processed/", mode=0o777)
			...
			outfil = periodpath + "/subscription_processed/477_" + isp + "_subscription_processed.csv"
			sql = """COPY (Select tract,
			...
			if voipneeded == True:
				...
				outfil = periodpath + "/subscription_processed/" + isp + "_voice_subscription_processed.csv"
				isExist = os.path.exists(outfil)
				if (isExist):
					os.remove(outfil)
				...
				sleep(5)
				sql = 'select distinct substring(tract,1,2) as statefips from subscribers.subs_' + str(isp) 
				print(sql)
				ps_cursor.execute(sql)
				...
				for state in states:
					...
					if techsums is not None:
						for t in techsums:
							contents += "tech code " + str(t[0]) + ": " + str(t[1]) + "\n" 
						with open(outfil,"a") as ts:
							print(contents, file=ts)

			# Update final status to complete
			sql = """Update filer_processing_status set subscription_processed = true, subscription_status = 'complete' where org_id = """ + isp + """ and filing_period = '""" + period + """' """;
			cursor.execute(sql)
			conn.commit()
...
conn = psycopg2.connect(
   database="broadband", user='broadband', password='thecomputerhasbutterinit', host='localhost', port= '5432'
)
					...
					for period in os.listdir(periodpath):
						if os.path.isdir(os.path.join(periodpath, period)):
							...
							if dirname == 'subscribers' and endperiod == per:
								print("          building subscription file from subscribers for isp ",procisp) 
								#go build subscription file
								subpath = periodpath + '/' + os.path.basename(period)
								print("subpath",subpath);
								for subfile in os.listdir(subpath):
									print("subfile",subfile)
									if os.path.isfile(os.path.join(subpath, subfile)):
										sfile = os.path.basename(subfile)
										print("sfile",sfile)
										print("          processing subscribers file ", subfile)
										create_subscription(subfile,sfile,procisp,periodpath,endperiod)
