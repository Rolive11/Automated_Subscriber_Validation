...
def validate_general_columns(cleaned_df, errors, corrected_cells, flagged_cells):
    ...
    # PRE-PROCESS: Convert 'dsl' to 'adsl2' BEFORE any validation
    # This ensures all validation logic sees 'adsl2' instead of 'dsl'
    if 'technology' in cleaned_df.columns:
        for idx in range(len(cleaned_df)):
            tech = cleaned_df.loc[idx, 'technology']
            if pd.notna(tech):
                tech_normalized = str(tech).lower().strip()
                if tech_normalized == 'dsl':
                    orig_row = cleaned_df["OrigRowNum"][idx]
                    cleaned_df.loc[idx, 'technology'] = 'adsl2'
                    ...
                    debug_print(f"Pre-processed 'dsl' to 'adsl2' for OrigRowNum={orig_row}")
    ...
    for col in cleaned_df.columns:
        ...
        if col in REQUIRED_COLUMNS:
            ...
            for idx, (val, blank) in enumerate(zip(values, is_blank)):
                if blank:
                    ...
                    # Skip download/upload requirement for VoIP technology
                    if col in ["download", "upload"]:
                        tech = cleaned_df.loc[idx, "technology"]
                        if pd.notna(tech) and str(tech).lower().strip() == "voip":
        ...
        for idx, val in enumerate(values):
            ...
            if col == "customer":
                ...
                # If technology is voip, remove speeds (VoIP doesn't have internet speeds)
                tech = cleaned_df.loc[idx, "technology"]
                if pd.notna(tech) and str(tech).lower().strip() == "voip":
                    cleaned_df.loc[idx, col] = pd.NA
                    debug_print(f"Removed {col} speed for VoIP subscriber at OrigRowNum={orig_row}")
                    continue
